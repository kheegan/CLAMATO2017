...
for ii=0, nsel-1 do begin
   ...
   if qsoflag EQ 0 then begin
      ...
      masklines = where((wave GT (1175.7*(1.+ztmp)-10.) AND $
                         wave LT (1175.7*(1.+ztmp)+10.)) OR $
                        (wave GT (1134.4*(1.+ztmp)-10.) AND $
                         wave LT (1134.4*(1.+ztmp)+10.))OR $
                        (wave GT (1085.7*(1.+ztmp)-10.) AND $
                         wave LT (1085.7*(1.+ztmp)+10.)) )
   ...
   if ncont EQ 1 then begin
      ...
      ;; for galaxies we just do a power-law multiplied
      ;; by a factor implied by Andreu Arinyo's analysis of
      ;; Starburst99 spectra, while for quasars we do MF-PCA
...
;   read, dummy
   ...
   ;; fvec is simply mean-flux as a function wavelength
   ...
   ;; This line needs to be commented out if rebinning...
   ;zerow = where(wtmp EQ 0, nzero)
   ;if nzero GT 0 then remove, zerow, delta_tmp, noise_tmp, losvec, wtmp
   
   npix_tmp = n_elements(noise_tmp)

   xout_tmp = replicate(xtmp, npix_tmp)
   yout_tmp = replicate(ytmp, npix_tmp)
   zout_tmp = losvec

   ;;Rebinning... also comment out REMOVE above  --------
   binfac=2.
   npix_rebin = round(npix_tmp/binfac)
   xout_tmp = congrid(xout_tmp, npix_rebin)
   yout_tmp = congrid(yout_tmp, npix_rebin)
   zout_tmp = congrid(zout_tmp, npix_rebin)
   delta_tmp = rebin_spectrum(delta_tmp,losvec,zout_tmp)/binfac
   wtmp = rebin_spectrum(wtmp, losvec, zout_tmp)
   ;; --------------------------------------------------
   
   delta_out = [delta_out, delta_tmp]
   noise_out =[noise_out , noise_tmp]
   weights_out = [weights_out, wtmp]
   weightlist = [weightlist, avgweight]

   x_out = [x_out, xout_tmp]
   y_out = [y_out, yout_tmp]
   z_out = [z_out, zout_tmp]
...
openw, 11, 'pixel_x.bin'
writeu, 11, x_out
close, 11

openw, 12, 'pixel_y.bin'
writeu, 12, y_out
close, 12

openw, 13, 'pixel_z.bin'
writeu, 13, z_out
close, 13

openw, 14, 'pixel_weights.bin'
writeu, 14, weights_out
close, 14

openw, 12, 'pixel_delta.bin'
writeu, 12, delta_out
close, 12
